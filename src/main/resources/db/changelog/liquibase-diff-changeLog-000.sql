...
CREATE SEQUENCE passanger_info_id_seq MINVALUE 1 START WITH 1 INCREMENT BY 1 CACHE 2;
CREATE TABLE SGV_PASSANGER_INFO (
    ID NUMERIC(19,0) NOT NULL,
    INSTITUTION NUMERIC(19,0) NOT NULL,
    CONSTRAINT SGV_PASSAN_PK PRIMARY KEY (ID)
);

ALTER TABLE SGV_PASSANGER_INFO ADD CONSTRAINT FK_PASSANGER_INSTITUTION FOREIGN KEY (INSTITUTION) REFERENCES SGV_INSTITUTION(ID);

--rollback DROP TABLE SGV_PASSANGER_INFO;
--rollback DROP SEQUENCE passanger_info_id_seq;
...
CREATE TABLE SGV_USER (
    ...
    ADDRESS NUMERIC(19,0) NOT NULL,
    PASSANGER_INFO NUMERIC(19,0),
    DRIVER_INFO NUMERIC(19,0),
    CONSTRAINT SGV_USER_PK PRIMARY KEY (ID)
);

ALTER TABLE SGV_USER ADD CONSTRAINT FK_USER_PASSANGER_INFO FOREIGN KEY (PASSANGER_INFO) REFERENCES SGV_PASSANGER_INFO(ID);
...
CREATE TABLE SGV_ROUTE_PASSANGER (
	...
	ID_PASSANGER_INFO NUMERIC(19) NOT NULL,
	CONSTRAINT SGV_ROUTE_PASSANGER_PK PRIMARY KEY (ID_ROUTE, ID_PASSANGER_INFO)
);

ALTER TABLE SGV_ROUTE_PASSANGER ADD CONSTRAINT FK_RP_ROUTE FOREIGN KEY (ID_ROUTE) REFERENCES SGV_ROUTE(ID);
ALTER TABLE SGV_ROUTE_PASSANGER ADD CONSTRAINT FK_RP_PASSANGER FOREIGN KEY (ID_PASSANGER_INFO) REFERENCES SGV_PASSANGER_INFO(ID);
